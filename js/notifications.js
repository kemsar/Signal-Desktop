  ...
  Whisper.Notifications = new (Backbone.Collection.extend({
    ...
    update() {
      ...
      if (
        ...
        const audioSource =
          storage.get('audio-notification-file') !== undefined
            ? storage.get('audio-notification-file')
            : 'notifications/graceful.ogg';
        window.log.info(
          `Platform is linux...manually playing notification sound: '${audioSource}'`
        );
        const noise = new Audio(audioSource);
        noise.play().catch(error => {
          window.log.error(
            `Failed to play notification sound: '${
              error.message
            }'. Trying default notification sound...`
          );
          const dNoise = new Audio('notifications/graceful.ogg');
          dNoise.play().catch(error2 => {
            window.log.error(
              `Failed to play default notification sound, as well: '${
                error2.message
              }'`
            );
          });
